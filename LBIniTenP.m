...
data41=readMercuryCG('stat\LBIniTP\S004M1LBIni.stat');
...
data52=readMercuryCG('stat\LBIniTP\S005M5LBIni.stat');
data53=readMercuryCG('stat\LBIniTP\S005M10LBIni.stat');
...
Qithree=[0.00503094368794659,0.0237306400304235,0.0389777166480000];
Qi=Qithree(3);
...
subplot(2,2,1)
for i=1:size(Qsm,1)
plot(Liquid,Qsm(length(Shields)-i+1,:),'o','LineWidth', 1);
hold on
end
plot(Liquid,mean(Qsm),'k-');
ylim([0 0.07]);
...
subplot(2,2,2)
for i=1:size(Qsm,1)
plot(Liquid,Cs(length(Shields)-i+1,:),'o','LineWidth', 1);
hold on
end
...
subplot(2,2,3)
for i=1:size(Qsm,1)
plot(Liquid,Ups(length(Shields)-i+1,:),'o','LineWidth', 1);
hold on
end
plot(Liquid,mean(Ups),'k-');
...
subplot(2,2,4)
for i=1:size(Qsm,1)
plot(Liquid,Hs(length(Shields)-i+1,:),'o','LineWidth', 1);
hold on
end
...
ylim([0, 0.7]);xlim([0, 5]);
text(0.05, 0.95, title_index(i), 'Units', 'normalized', 'FontSize', 12);
...
plot(t(134),fac_dim*Q64(134),'*','color',[0.6350 0.0780 0.1840]);
subplot(4,2,index(7))
set(gca, 'ColorOrder', colors, 'NextPlot', 'replacechildren');
for j=0:4
   eval(['plot([0 t], [Cini Cvx6',num2str(j),']);']); 
   hold on
end
plot(t(134),Cvx64(134),'*','color',[0.6350 0.0780 0.1840]);
box on
ylim([0 0.5]);
text(0.05, 0.95, '(g)', 'Units', 'normalized', 'FontSize', 12);
xlabel('$t$ [s]','Interpreter','Latex','fontsize',12);ylabel('$C_\mathrm{sal}$ [kg/m$^2$]','Interpreter','Latex','fontsize',12);
title('$\tilde{\Theta}$=0.06', 'Interpreter', 'Latex');
subplot(4,2,index(8))
set(gca, 'ColorOrder', colors, 'NextPlot', 'replacechildren');
for j=0:4
   eval(['plot([0 t], [Uini Upx6',num2str(j),']);']); 
   hold on
end
plot(t(134),Upx64(134),'*','color',[0.6350 0.0780 0.1840]);
box on
text(0.05, 0.95, '(h)', 'Units', 'normalized', 'FontSize', 12);
xlabel('$t$ [s]','Interpreter','Latex','fontsize',12);ylabel('$U_\mathrm{sal}$ [m/s]','Interpreter','Latex','fontsize',12);
title('$\tilde{\Theta}$=0.06', 'Interpreter', 'Latex');
%highlighting the last three in a box
pos = subplot(4,2,8).Position;
annotation('rectangle', ...
               [pos(1)-0.08, pos(2)-0.05, pos(3)+0.12, pos(4)+0.51], ...
               'Color', 'black', 'LineWidth', 1.5);
           
%T_peak,T_decline,Q_peak - Omega           
figure 
subplot(1,3,1)
for i = 1:6
...
ylim([0 3.5]);
text(0.85, 0.95, '(a)', 'Units', 'normalized', 'FontSize', 12);
...
text(0.85, 0.95, '(b)', 'Units', 'normalized', 'FontSize', 12);
xlabel('$\Omega$ [$\%$]','Interpreter','Latex','fontsize',12);ylabel('$T_{\mathrm{decline}}$ [s]','Interpreter','Latex','fontsize',12);
legend('$\tilde{\Theta}$=0.06','$\tilde{\Theta}$=0.05','$\tilde{\Theta}$=0.04','$\tilde{\Theta}$=0.03','$\tilde{\Theta}$=0.02', '$\tilde{\Theta}$=0.01', 'Interpreter','Latex', 'fontsize',10, 'Location','best');
subplot(1,3,3)
for i = 1:6
plot(Liquid, Qpeak(7-i,:), 'o', 'LineWidth', 1);
hold on
end
text(0.85, 0.95, '(c)', 'Units', 'normalized', 'FontSize', 12);
xlabel('$\Omega$ [$\%$]','Interpreter','Latex','fontsize',12);ylabel('$Q_{\mathrm{peak}}$ [kg/m/s]','Interpreter','Latex','fontsize',12);
...
legend('$\Omega$=0','$\Omega=1\%$','$\Omega=5\%$','$\Omega=10\%$','$\Omega=20\%$','Interpreter','Latex', 'fontsize',8);
subplot1_position = get(subplot1, 'Position');
inset_position = [subplot1_position(1) + 0.2 * subplot1_position(3), ... % left
                  subplot1_position(2) + 0.425 * subplot1_position(4), ... % bottom
                  0.35 * subplot1_position(3), ...                       % width
                  0.3 * subplot1_position(4)];                          % height
...
errorbar(inset_axes,[0.43, 0.475],[105, 82.5],zeros(1,2),zeros(1,2),[0.03, 0.085],[0.03, 0.085],'x', 'LineWidth', 1);
hold on;
...
legend('Wet data S24','S24 formula','This study');
%%plot Lpeak - Qi
subplot(1,2,2);
box on
...
legend('$N_\mathrm{ip}$=1','$N_\mathrm{ip}$=5','$N_\mathrm{ip}$=10','Interpreter','Latex','fontsize',8);
